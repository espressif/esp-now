...
esp_err_t light_driver_init(light_driver_config_t *config)
{
    ...
    if (esp_storage_get(LIGHT_STATUS_STORE_KEY, &g_light_status, sizeof(light_status_t)) != ESP_OK) {
...
esp_err_t light_driver_set_rgb(uint8_t red, uint8_t green, uint8_t blue)
{
    ...
    ESP_ERROR_RETURN(ret < 0, ret, "led_pwm_set_channel, ret: %d", ret);
...
{
    ESP_PARAM_CHECK(hue <= 360);
    ESP_PARAM_CHECK(saturation <= 100);
    ESP_PARAM_CHECK(value <= 100);
    ...
    ESP_ERROR_RETURN(ret < 0, ret, "light_driver_hsv2rgb, ret: %d", ret);
    ...
    ret = esp_storage_set(LIGHT_STATUS_STORE_KEY, &g_light_status, sizeof(light_status_t));
    ESP_ERROR_RETURN(ret < 0, ret, "esp_storage_set, ret: %d", ret);
...
{
    ESP_PARAM_CHECK(brightness <= 100);
    ESP_PARAM_CHECK(color_temperature <= 100);
...
{
    ESP_PARAM_CHECK(color_temperature);
    ESP_PARAM_CHECK(brightness);
...
esp_err_t light_driver_set_switch(bool on)
{
    ...
    if (!g_light_status.on) {
        ret = led_pwm_set_channel(CHANNEL_ID_RED, 0, g_light_status.fade_period_ms);
        ESP_ERROR_RETURN(ret < 0, ESP_FAIL, "led_pwm_set_channel, ret: %d", ret);
        ...
        switch (g_light_status.mode) {
            case MODE_HSV:
                ...
                ESP_ERROR_RETURN(ret < 0, ESP_FAIL, "light_driver_set_hsv, ret: %d", ret);
                break;
            ...
            case MODE_CTB:
                ...
                ESP_ERROR_RETURN(ret < 0, ESP_FAIL, "light_driver_set_ctb, ret: %d", ret);
                break;
    ...
    ret = esp_storage_set(LIGHT_STATUS_STORE_KEY, &g_light_status, sizeof(light_status_t));
    ESP_ERROR_RETURN(ret < 0, ESP_FAIL, "esp_storage_set, ret: %d", ret);
...
esp_err_t light_driver_breath_start(uint8_t red, uint8_t green, uint8_t blue)
{
    ...
    ESP_ERROR_RETURN(ret < 0, ESP_FAIL, "led_pwm_start_blink, ret: %d", ret);
...
esp_err_t light_driver_breath_stop()
{
    ...
    ESP_ERROR_RETURN(ret < 0, ESP_FAIL, "led_pwm_stop_blink, ret: %d", ret);
...
esp_err_t light_driver_fade_brightness(uint8_t brightness)
{
    ...
    if (g_light_status.mode == MODE_HSV) {
        ...
        if (brightness != 0) {
            ...
            ESP_ERROR_RETURN(ret < 0, ESP_FAIL, "led_pwm_get_channel, ret: %d", ret);
