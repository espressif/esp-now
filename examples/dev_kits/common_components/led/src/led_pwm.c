...
esp_err_t led_pwm_regist_channel(ledc_channel_t channel, gpio_num_t gpio_num)
{
    ...
    ESP_ERROR_RETURN(ret != ESP_OK, ret, "LEDC channel configuration");
...
{
    ESP_ERROR_RETURN(g_light_config == NULL, ESP_ERR_INVALID_STATE, "iot_led_init() must be called first");
    ESP_ERROR_RETURN(dst == NULL, ESP_ERR_INVALID_ARG, "dst should not be NULL");
...
{
    ESP_ERROR_RETURN(g_gamma_table == NULL, ESP_ERR_INVALID_STATE, "iot_led_init() must be called first");
