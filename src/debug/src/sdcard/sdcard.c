...
esp_err_t sdcard_init(const sdcard_config_t *config)
{
    ...
    gpio_set_pull_mode(config->gpio_num_cmd, GPIO_PULLUP_ONLY);   /**< CMD, needed in 4- and 1- line modes */
    gpio_set_pull_mode(config->gpio_num_d0, GPIO_PULLUP_ONLY);    /**< D0, needed in 4- and 1- line modes */
    gpio_set_pull_mode(config->gpio_num_d1, GPIO_PULLUP_ONLY);    /**< D1, needed in 4-line mode only */
    gpio_set_pull_mode(config->gpio_num_d3, GPIO_PULLUP_ONLY);   /**< D2, needed in 4-line mode only */
    gpio_set_pull_mode(config->gpio_num_d3, GPIO_PULLUP_ONLY);   /**< D3, needed in 4- and 1- line modes */
    ...
    ESP_ERROR_RETURN(ret != ESP_OK, ret, "Failed to initialize the card"
    ...
    if (stat(file_path, &st) == ESP_OK) {
        ...
        while ((entry = readdir(dirptr)) != NULL) {
            ...
            if (stat(file_path, &st) == 0) {
                ESP_LOGI(TAG, "remove file: %14s", entry->d_name);
                unlink(file_path);
            }
        }
...
esp_err_t sdcard_rename_file(const char *file_name_old, const char *file_name_new)
{
    ...
    esp_err_t ret = rename(file_path_old, file_path_new);
    ESP_ERROR_RETURN(ret != ESP_OK, ret, "Rename failed, old: %s, new: %s",
...
esp_err_t sdcard_list_file(const char *file_name)
{
    ...
    if (stat(file_path, &f_stat) == ESP_OK) {
        unlink(file_path);
        return ESP_OK;
    }

    if (strcmp(file_name, "*") && strncmp(file_name, "*.", 2)) {
        ESP_LOGW(TAG, "file_name: %s", file_name);
        return ESP_FAIL;
    }
    ...
    for (int count = 0; (entry = readdir(dirptr)) != NULL; ++count) {
        ...
        ESP_LOGI(TAG, "<%02d>  %s  %7ld  %s", count, timebuf, f_stat.st_size, entry->d_name);
    }
...
{
    ESP_PARAM_CHECK(file_name);
    ESP_PARAM_CHECK(type != FILE_TYPE_NONE);
...
#define BYTES_PER_LINE 16
    ...
    char file_path[SDCARD_FILE_NAME_MAX_LEN] = { 0 };
    sprintf(file_path, SDCARD_BASE_PATH "/%s", file_name);
    uint8_t *buffer = ESP_MALLOC(BUF_MAX_LEN);

    FILE *fp = fopen(file_path, "r");

    if (fp == NULL) {
        ESP_LOGE(TAG, "%s no exist", file_name);
        fclose(fp);
    ...
    do {
        ...
        switch (type) {
            ...
            case FILE_TYPE_BASE64: {
                ...
                size_t b64_olen = 0;
                uint8_t *b64_buf = ESP_CALLOC(b64_buf_size, sizeof(uint8_t));
...
esp_err_t sdcard_write_file(const char *file_name, uint32_t offset, const void *data, size_t size)
{
    ...
    ESP_ERROR_GOTO(fp == NULL, EXIT, "Create file %s failed", file_name);

    if (offset == UINT32_MAX) {
        ret = fseek(fp, 0, SEEK_END);
        ESP_ERROR_GOTO(ret != ESP_OK, EXIT, "seek packet error");
    } else {
    ...
    ESP_ERROR_GOTO(real_write != size, EXIT, "Write packet error");
...
esp_err_t sdcard_read_file(const char *file_name, uint32_t offset, uint8_t *data, size_t *size)
{
    ...
    *size = fread(data, 1, *size, fp);
    ESP_ERROR_GOTO(*size <= 0, EXIT, "Read packet error");
