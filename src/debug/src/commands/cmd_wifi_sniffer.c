...
#define CONFIG_SNIFFER_TASK_PRIORITY     tskIDLE_PRIORITY + 1
...
static esp_err_t create_packet_file(void)
{
    ...
    ESP_ERROR_RETURN(!pcap_config.fp, ESP_FAIL, "Create file %s failed", filename);
...
static uint32_t search_wifi_filter_hashtable(const char *key)
{
    ...
    while (strncmp(wifi_filter_hash_table[idx].filter_name, key, len)) {
        ...
        ESP_ERROR_RETURN(idx == start_idx, 0, "wrong key");
    }
...
static void sniffer_task(void *parameters)
{
    ...
    while (sniffer_running) {
        ...
        ESP_ERROR_CONTINUE(ret != pdTRUE, "xQueueReceive timeout");
...
static int wifi_sniffer_func(int argc, char **argv)
{
    ...
    if (wifi_sniffer_args.stop->count) {
        if (sniffer_running) {
            ...
            ESP_LOGW(TAG, "sniffer already stop");
        }
    ...
    if (!sniffer_running) {
        ...
        ESP_LOGW(TAG, "Please stop sniffer firstly, then start with parameters");
    }
