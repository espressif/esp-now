...
{
    ESP_ERROR_RETURN(!file, ESP_FAIL, "file empty");
    ...
    ESP_ERROR_RETURN(real_write != 1, ESP_FAIL, "Write packet header error");
    real_write = fwrite(payload, sizeof(uint8_t), length, file);
    ESP_ERROR_RETURN(real_write != length, ESP_FAIL, "Write packet payload error");
...
{
    ESP_ERROR_RETURN(!file, ESP_OK, "file not exist, or already close");
...
esp_err_t pcap_new(pcap_config_t *config)
{
    ...
    ESP_ERROR_GOTO(real_write != 1, err_write, "Write Pcap file header error");
