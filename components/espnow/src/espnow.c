...
/* Keep the type order same with espnow_type_t */
static espnow_recv_handle_t g_recv_handle[ESPNOW_TYPE_MAX];
...
void espnow_recv_cb(const uint8_t *addr, const uint8_t *data, int size)
{
    ...
    if (espnow_data->version != ESPNOW_VERSION || (espnow_data->type >= ESPNOW_TYPE_MAX)
    ...
    if (espnow_data->type == ESPNOW_TYPE_ACK) {
        ...
        *magic = frame_head->magic;
        ...
        goto EXIT;
    } else if (espnow_data->type == ESPNOW_TYPE_GROUP) {
                      ...
                      size_t size, const espnow_frame_head_t *data_head, TickType_t wait_ticks)
{
    ...
    if (g_espnow_config->sec_enable
        && type != ESPNOW_TYPE_ACK && type != ESPNOW_TYPE_FORWARD
...
static void espnow_main_task(void *arg)
{
    ...
    if (g_espnow_config && g_espnow_config->qsize) {
        ...
        if (g_recv_handle[ESPNOW_TYPE_ACK].enable) {
...
{
    ESP_ERROR_RETURN(!g_espnow_config, ESP_ERR_ESPNOW_NOT_INIT, "ESPNOW is not initialized");
    ESP_PARAM_CHECK(type >= ESPNOW_TYPE_ACK && type < ESPNOW_TYPE_MAX);
...
esp_err_t espnow_set_key(uint8_t key_info[APP_KEY_LEN])
{
    ...
    ret = esp_storage_set("key_info", key_info, APP_KEY_LEN);
...
esp_err_t espnow_get_key(uint8_t key_info[APP_KEY_LEN])
{
    ...
    return esp_storage_get("key_info", key_info, APP_KEY_LEN);
}

esp_err_t espnow_erase_key(void)
{
    return esp_storage_erase("key_info");
}
